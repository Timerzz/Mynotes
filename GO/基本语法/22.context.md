...
## Doc：
1. context.Background() Context 创建根节点
2. context.WithCancel(parent Context) 返回用于创建子节点的context和取消函数
```go
ctx, cancel := context.WithCancel(context.Background())
```
3. context.WithDeadline(parent Context, d time.Time)返回用于创建**到某个时间**后就取消任务的子节点的context和取消函数
```go
ctx, cancel := context.WithDeadline(context.Background(), time.Now().Add(2 * time.Second))
```
4. context.WithTimeout(parent Context, timeout time.Duration)返回用于创建**过了一定时间**后取消任务的子节点的context和取消函数
```go
ctx, cancel := context.WithTimeout(context.Background(), 2 * time.Second)
```
- 通过ctx.Done可以获得父节点是否取消任务
